...
class OptimizedThermalYOLO:
    ...
    def _apply_speed_optimizations(self, model: YOLO) -> YOLO:
        ...
        # 2. Model pruning (structured to maintain accuracy)
        model = self._apply_structured_pruning(model, pruning_ratio=0.3)
        
        # 3. Quantization for inference
        if hasattr(model, 'model'):
            try:
                model.model = torch.quantization.quantize_dynamic(
                    model.model, {nn.Conv2d, nn.Linear}, dtype=torch.qint8
                )
                print("✅ Dynamic quantization applied")
            except:
                print("⚠️  Quantization failed")
    ...
    def _get_optimal_params(self) -> Dict[str, Any]:
        ...
        if self.optimization_level == "speed":
            ...
            return {
                'conf': 0.15,    # Very low confidence for max recall
    ...
    def detect_threats_optimized(
        ...
        if results and len(results) > 0 and results[0].boxes is not None:
            ...
            for i in range(len(boxes)):
                ...
                threat_classes = {
                    0: "hostile_personnel", 1: "bicycle", 2: "hostile_vehicle",
                    3: "motorcycle", 4: "transport_vehicle", 5: "train",
                    6: "heavy_vehicle", 7: "infrastructure", 8: "aircraft",
                    9: "watercraft"
                }
                
                class_name = threat_classes.get(class_id, f"unknown_threat_{class_id}")
